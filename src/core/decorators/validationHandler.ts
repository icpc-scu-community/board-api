...
export function validationHandler({ params = {}, query = {}, body = undefined }: IValidationRules): RequestHandler {
  return (req: Request, _: Response, next: NextFunction): void => {
    // check for invalid params first, fails => not found
    const paramsSchema = Joi.object(params);
    const paramsValue = req.params;
    const { error: paramsError } = paramsSchema.validate(paramsValue, JOI_VALIDATION_OPTIONS);
    if (paramsError) {
...
export function toErrorResponse(errDetails: Joi.ValidationErrorItem[]): IErrorResponse[] {
